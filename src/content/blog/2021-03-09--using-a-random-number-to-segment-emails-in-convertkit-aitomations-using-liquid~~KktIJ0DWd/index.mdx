...
If you want to generate a (pseudo) random number using a Liquid filter in [Convertkit](https://partners.convertkit.com/4xxwjvqn59fl), here's the code:
...
My specific use case for this was sending automations to a fraction of a group for testing because [ConvertKit](https://partners.convertkit.com/4xxwjvqn59fl) automation do not have a built in way to accomplish this.
...
I'll start with 2%, then up that to 8%, 10%, 20%, 50%, 80%, 100% just reflowing subscribers through the automation as needed since we are guaranteed in [Convertkit](https://partners.convertkit.com/4xxwjvqn59fl) that any given email won't be sent twice if it's in a Sequence.
